...
export default function AIDailyBriefing({ tasks, projects, onUpdateTask, onDeleteTask }: Props) {
  ...
  const handleGenerate = async () => {
    setLoading(true); setError(null); setBriefing(null); setAppliedStale(new Set());
    try {
      ...
      const data = await aiRequest<DailyFocusResponse>('/api/tasks/ai-daily-focus', { tasks: snapshots, today });
      setBriefing(data);
    } catch (err: any) {
                  ...
                  {briefing.topTasks.map((ft: FocusTask) => {
                    ...
                    return (
                      <div key={ft.taskId} className="flex items-start space-x-3 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3">
                        <div className="flex-shrink-0 w-7 h-7 rounded-full bg-blue-600 text-white text-sm font-bold flex items-center justify-center">{ft.rank}</div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center space-x-2 flex-wrap gap-y-1">
                            <p className="text-sm font-semibold text-gray-900">{ft.taskTitle}</p>
                            {task && <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${priorityColor[task.priority]}`}>{task.priority}</span>}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {briefing.staleTasks.map((st: StaleTask) => (
                      <div key={st.taskId}
                        className={`flex items-start justify-between bg-gray-50 border rounded-xl px-4 py-3 transition-opacity ${ appliedStale.has(st.taskId) ? 'opacity-40' : '' }`}>
